...
# object size 700x750
par(mfrow = c(3, 2), mar = c(4, 4, 1, 0.5), oma = c(1.5, 2, 2, 0))
...
barplot(
  as.matrix(check.summary[check.summary$MIxS, -c(1, 2, ncol(check.summary))]),
  col = levels(check.summary$col),
  legend.text = paste0(levels(check.summary$target.gene), " (", round(rowSums(check.summary[check.summary$MIxS, -c(1, 2, ncol(check.summary))])/sum(rowSums(check.summary[check.summary$MIxS, -c(1, 2, ncol(check.summary))])) * 100), "%)"),
  args.legend = list(x = "topleft", cex = 1.2, bty = "n", pt.cex = 1.5),
  names.arg = colnames(check.summary)[-c(1, 2, ncol(check.summary))],
  las = 2,
  cex.axis = 1,
